...
def testModel(x_test, weight, bias):
    f = open("datas/ouput.csv", 'w', encoding='utf-8', newline="")
...
def save_Pre(weight, bias):
    f = open("datas/pre.csv", 'w', encoding='utf-8')
    csv_write = csv.writer(f)
...
def test():
    ...
    test_data = pd.read_csv("datas/test.csv", header=None, usecols=np.arange(2, 11).tolist())
...
def train():
    df = pd.read_csv("datas/train.csv",
                     usecols=np.arange(3, 27).tolist(),
                     encoding='big5')  # np.arange(3, 27)生成一个3-27的一组数，tolist()是把其转换成列表形式[3,4,...,27]
